...
//  FlickrPhotoSearch
//
//  Created by Andrew Ikenna Jones on 12/13/21.
//
...
enum FlickrPublicAPI  {
    static let host: String = "api.flickr.com"
    static let path: String = "/services/feeds/photos_public.gne"
    enum Tags {
        static let puppy = "puppy"
    }
}

protocol Flickr_Service_Protocol {
    func searchPhoto(with term: SearchTerm, completion: @escaping (Result<[FlickrPhoto], Error>) -> Void)
    func publicPuppies(completion: @escaping (Result<[FlickrPhoto], Error>) -> Void)
}
...
final class FlickrService: Flickr_Service_Protocol {
    ...
    func searchPhoto(with term: SearchTerm, completion: @escaping (Result<[FlickrPhoto], Error>) -> Void) {
        ...
        let puppyTag = URLQueryItem(name: "tags", value: FlickrPublicAPI.Tags.puppy)

        urlComponents.queryItems = [methodQuery, apiKeyQuery, jsonFormatQuery, noJsonCallbackQuery, puppyTag, perPageQuery, sortQuery]
    ...
    func publicPuppies(completion: @escaping (Result<[FlickrPhoto], Error>) -> Void) {
        var urlComponents = URLComponents()
        urlComponents.scheme = "https"
        urlComponents.host = FlickrPublicAPI.host
        urlComponents.path = FlickrPublicAPI.path

        let jsonFormatQuery = URLQueryItem(name: "format", value: "json")
        let noJsonCallbackQuery = URLQueryItem(name: "nojsoncallback", value: "1")
        let puppyTag = URLQueryItem(name: "tags", value: FlickrPublicAPI.Tags.puppy)
        urlComponents.queryItems = [jsonFormatQuery, noJsonCallbackQuery, puppyTag]
